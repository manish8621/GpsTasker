...
class LocationClient(private val context: Context,private val fusedLocationProviderClient: FusedLocationProviderClient) {
     ...
     * */
    fun getCurrentLocationUpdates(oneShot:Boolean,onSuccess:(location:Location)->Unit){
        //check location is accessible
        if((checkLocationPermission() && checkLocationEnabled()).not())
